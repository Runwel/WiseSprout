...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $sql = "INSERT INTO users (email, username, password, status, verification_token) VALUES (?, ?, ?, 'pending', ?)";
        
        // Prepare and bind parameters
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssss", $email, $username, $hashedPassword, $verificationToken); // Bind username parameter

        // Execute the insert statement
        if ($stmt->execute()) {
            // Construct the message body with verification link
            $verificationLink = 'http://localhost/WaterSprinkles/auth/verify.php?email=' . urlencode($email) . '&token=' . urlencode($verificationToken);
            $messageBody = '
                <h1>Welcome to Sprout!</h1>
                <p>You have taken the first step towards a greener future. With Sprout, you can control your water sprinkler system efficiently, saving water and promoting healthier plants.</p>
                <p>Here are some benefits of using Sprout:</p>
                <ul>
                    <li>Save water by scheduling sprinkler operation based on plant needs.</li>
                    <li>Monitor soil moisture levels to prevent over or under watering.</li>
                    <li>Easy-to-use interface for setting up schedules and viewing sensor data.</li>
                    <li>Receive notifications and alerts about your garden\'s condition.</li>
                </ul>
                <p>We\'re excited to have you on board! To begin the verification process and semi-activate your account, please click the following link:</p>
                <p><a href="' . $verificationLink . '">Start Your Account Verification</a></p>
                <p>After clicking the link, your account will be semi-verified. To complete the full verification process:</p>
                <ol>
                    <li>Log in to your semi-verified account</li>
                    <li>Navigate to your profile settings</li>
                    <li>Complete any additional required information</li>
                    <li>Submit for final verification</li>
                </ol>
                <p>An administrator will review your information to fully activate your account.</p>
                <p>Happy gardening!</p>
            ';
            ...
            try {
                ...
                $mail->SMTPAuth = true;
                $mail->Username = 'prepperlife.now@gmail.com';
                $mail->Password = 'ysasrmyoonzvrooq';
                $mail->SMTPSecure = 'tls';
                $mail->Port = 587;

                // Recipients
                $mail->setFrom('prepperlife.now@gmail.com', 'Wise Sprout');
                $mail->addAddress($email); // Add a recipient

                // Content
                $mail->isHTML(true);
                $mail->Subject = 'Activate Your Wise Sprout Account';
                ...
                $errors[] = "Message could not be sent. Mailer Error: " . htmlspecialchars($mail->ErrorInfo);
            }
        } else {
...
<body>
    <main role="main" class="container" style="height: 100vh;">
        <div class="starter-template text-center">
            <img src="assets/pictures/WiseSprout.png" alt="WiseSprout" style="height: 200px; width: 200px;">
            <?php 
            // Display errors
            if (!empty($errors)) {
            ...
            <form class="container" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                ...
                <div class="form-group">
                    ...
                    <div id="password-requirements" class="text-left" style="display: none;">
                        <small>Password must contain at least 8 characters, including a number, a special character, an uppercase letter, and a lowercase letter.</small>
                    </div>
                </div>
                <div class="form-group">
    ...
    <script>
        document.getElementById('password').addEventListener('input', function() {
            ...
            if (!regex.test(password)) {
                ...
                this.setCustomValidity('Password must contain at least 8 characters, including a number, a special character, an uppercase letter, and a lowercase letter.');
            } else {
