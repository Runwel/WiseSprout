...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($days)) {
        ...
        $checkStatusQuery = "SELECT status FROM schedules WHERE id = ? AND user_id = ?";
        ...
        // If the status is ongoing, prevent the update and return an error response without opening the modal
        if ($status === 'Ongoing') {
            $response = [
                'success' => false,
                'message' => 'This schedule is currently ongoing and cannot be updated.'
            ];
        } else {
            ...
            if (isOverlap($start_time, $end_time, $user_id, $days, $conn, $id)) {
                ...
                $updateQuery = "UPDATE schedules SET status = 'Pending', start_time = ?, duration = ?, days = ? WHERE id = ? AND user_id = ?";
