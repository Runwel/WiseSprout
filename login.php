...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $sql = "SELECT id, username, password, status, email FROM users WHERE email = ?";
        
        // Prepare and bind parameters
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $email); // Bind username parameter
        ...
        if ($result->num_rows == 1) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                } elseif ($user['status'] == 'semi-verified') {
            ...
            <div class="starter-template text-center">
                ...
                <form class="container" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                    ...
                    <div class="d-flex justify-content-between">
                        <a href="forgot-password.php" class="mt-3">Forgot Password?</a>
                        <a href="register.php" class="mt-3">Create an account?</a>
                    </div>        
                </form>
            </div>
        </div>
